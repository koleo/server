...
static int dump_all_users()
{
  ...
  if (mysql_query_with_error_report(mysql, &tableres,
                                    "SELECT CONCAT(QUOTE(User), '@', QUOTE(Host)) AS u "
                                    "FROM mysql.user /*M!100005 WHERE is_role='N' */"))
    return 1;
  while ((row= mysql_fetch_row(tableres)))
  {
    if (opt_replace_into)
  ...
  if (mysql_roles_exist && mysql_query_with_error_report(mysql, &tableres,
      ...
      "   AND re.TO_HOST=re2.FROM_HOST"
      "   WHERE re.WITH_ADMIN_OPTION='Y'"
      "     AND re2.FROM_USER IS NULL"
      "   UNION SELECT c.n+1,"
      "                re.*"
      "   FROM create_role_order c"
      ...
      "   WHERE re.WITH_ADMIN_OPTION='Y' ) "
      "SELECT n, "
      "       CONCAT(QUOTE(FROM_USER), '/*M!100005 @', QUOTE(FROM_HOST), '*/') AS r,"
      "       CONCAT(QUOTE(TO_USER), IF(TO_HOST='', '', CONCAT('@', QUOTE(TO_HOST)))) AS u "
      "FROM create_role_order c "
      "ORDER BY n, "
      "         c.FROM_USER,"
      "         c.FROM_HOST,"
  ...
  while ((row= mysql_fetch_row(tableres)))
  {
    ...
    fprintf(md_result_file,
      ...
      opt_ignore ? "IF NOT EXISTS " : "", row[0]);
    fprintf(md_result_file, "/*M!100005 GRANT %s TO %s WITH ADMIN OPTION */;\n", row[0], row[1]);
  }
  mysql_free_result(tableres);
  ...
  if (mysql_roles_exist && mysql_query_with_error_report(mysql, &tableres,
      ...
      "FROM mysql.role_edges WHERE WITH_ADMIN_OPTION='Y'"))
    return 1;
  while ((row= mysql_fetch_row(tableres)))
  {
